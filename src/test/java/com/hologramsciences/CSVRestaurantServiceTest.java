...
public class CSVRestaurantServiceTest {
    ...
    public void getOpenRestaurantsReturnsResultsIncludesThai() {
        final List<Restaurant> openResults = csvRestaurantService.getOpenRestaurants(
                DayOfWeek.SATURDAY,
                LocalTime.of(0, 20));
        final List<String> restaurantNames = openResults.stream().map(Restaurant::getName)
    ...
    public void getOpenRestaurantsLateSaturdayIntoSunday() {
        final List<Restaurant> openResults = csvRestaurantService.getOpenRestaurants(
                DayOfWeek.SUNDAY,
                LocalTime.of(0, 20));
        final Set<String> restaurantNames = openResults.stream().map(Restaurant::getName)
                .collect(Collectors.toSet());
        final Set<String> expectedNames = new HashSet<>(
                Arrays.asList("The Cheesecake Factory", "Sudachi", "Thai Stick Restaurant",
                        "Sabella & La Torre", "Marrakech Moroccan Restaurant",
                        "Tim's Nighttime Banana Stand"));
    ...
    public void getOpenRestaurantsForLocalDateTimeReturnsResults() {
        ...
        final List<String> lateNightRestaurantNames = lateNightResults.stream()
                .map(Restaurant::getName)
                .collect(Collectors.toList());
        final List<String> expectedNames = Arrays.asList("Sudachi", "Thai Stick Restaurant",
    ...
    public void getOpenRestaurantsForLocalDateTimeReturnsResultsReallyLate() {
        ...
        final List<String> lightNightRestaurantNames = lateNightResults.stream()
    ...
    public void allRestaurantsHaveGoodOpenHours() {
        assertTrue(
                csvRestaurantService.getAllRestaurants().size() > 0 &&
                        csvRestaurantService.getAllRestaurants().stream().allMatch(
                                r -> r.getOpenHoursMap().size() > 0 && r.getOpenHoursMap().values()
                                        .stream()
                                        .allMatch(oh -> !oh.getStartTime().equals(oh.getEndTime())))
        );
    }
}
